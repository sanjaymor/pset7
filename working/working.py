...
def convert(s):
    time_result = re.fullmatch(
        r"(\d{1,2})(?::([0-5]\d))? (AM|PM) to (\d{1,2})(?::([0-5]\d))? (AM|PM)",
        s
    )
    ...
    if time_result:
        ...
        # Fixed: Aligned properly inside the 'if time_result:' block
        if not 1 <= int(time_12[0]) <= 12:
            raise ValueError

        if not 1 <= int(time_12[3]) <= 12:
            raise ValueError
        ...
        time_str = f'{str(time_24[0]).zfill(2)}:{str(time_24[1]).zfill(2)} to {str(time_24[2]).zfill(2)}:{str(time_24[3]).zfill(2)}'
        return time_str
